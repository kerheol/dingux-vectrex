...
# if defined( CAANOO_MODE ) || defined(GCW0_MODE)
...
# endif
  ...
  SDL_Surface *ovl_norm_surface = NULL;
  SDL_Surface *ovl_rot90_surface = NULL;
  SDL_Surface *blit_ovl_surface = NULL;
  ...
  if (DVE.dve_render_mode == DVE_RENDER_ROT90) {
    ...
    for (y = 0; y < SNAP_HEIGHT; y++) {
      for (x = 0; x < SNAP_WIDTH; x++) {
        scan_src_pixel = src_pixel + (BLIT_WIDTH * x * 3) + (SNAP_HEIGHT - y) * 3;
        *dst_pixel++ = *scan_src_pixel;
      }
    }
  } else {
    // 300x240 -> 80x100
    for (y = 0; y < SNAP_HEIGHT; y++) {
      scan_src_pixel = src_pixel + (BLIT_WIDTH * y * 3);
      for (x = 0; x < SNAP_WIDTH; x++) {
...
psp_sdl_save_screenshot(void)
{
  ...
  sprintf(TmpFileName,"%s/scr/screenshot_%d.png", DVE.dve_home_dir, DVE.psp_screenshot_id++);
  if (DVE.psp_screenshot_id >= 10) DVE.psp_screenshot_id = 0;
...
# if defined ( OD_MODE) || defined(GCW0_MODE)
...
# endif
  ...
  ovl_norm_surface = SDL_CreateRGBSurface(SDL_SWSURFACE,
  ...
  ovl_rot90_surface = SDL_CreateRGBSurface(SDL_SWSURFACE,
    PSP_SDL_SCREEN_WIDTH, PSP_SDL_SCREEN_HEIGHT,
    back_surface->format->BitsPerPixel,
    back_surface->format->Rmask,
    back_surface->format->Gmask,
  ...
  for (Index = 0; Index < DVE_MAX_SAVE_STATE; Index++) {
    DVE.dve_save_state[Index].surface =
...
psp_sdl_default_config()
{
  getcwd(DVE.rom_directory, sizeof(DVE.rom_directory));
  strcat(DVE.rom_directory , "/roms/");
}
...
psp_sdl_load_config()
{
  ...
  sprintf(filename, "%s/set/global.cfg", DVE.dve_home_dir);
  ...
  while (fgets(Buffer,512, FileDesc) != (char *)0) {
    ...
    *Scan = '\0';
    if (!strcasecmp(Buffer,"rom_directory")) {
      strcpy(DVE.rom_directory, Scan+1);
    }
  }
...
psp_sdl_save_config()
{
  ...
  if (FileDesc != (FILE *)0 ) {
    ...
    fprintf(FileDesc, "rom_directory=%s\n" , DVE.rom_directory);
    fclose(FileDesc);
