...
psp_fmgr_getExtId(const char *szFilePath)
{
  ...
  if((pszExt = strrchr(szFilePath, '.'))) {
    if ((!strcasecmp(pszExt, ".rom")) ||
        (!strcasecmp(pszExt, ".vec")) ||
        (!strcasecmp(pszExt, ".gam")) ||
...
psp_fmgr_ask_confirm(void)
{
  ...
  psp_sdl_back2_print(230,  70, "Delete a file",
                     PSP_MENU_WARNING_COLOR);
  psp_sdl_back2_print(210,  80, "press X to confirm !",
                     PSP_MENU_WARNING_COLOR);
  psp_sdl_flip();
...
psp_fmgr_menu(int format)
{
  ...
  if (first) {
    first = 0;
    sprintf(user_filedir_kbd, "%s/kbd/", DVE.dve_home_dir);
    sprintf(user_filedir_set, "%s/set/", DVE.dve_home_dir);
    sprintf(user_filedir_cht, "%s/cht/", DVE.dve_home_dir);
    user_filedir_rom = DVE.rom_directory;
  }
  ...
  if (psp_file_request(user_filename, user_filedir)) {
    ...
    else
    {
      ...
      if (file_format == FMGR_FORMAT_ZIP) {
        ...
        if (user_file_format == FMGR_FORMAT_ROM) /* Rom */ error = dve_rom_load(user_filename, 1);
      }
      else
      {
        if (file_format == FMGR_FORMAT_ROM) /* Rom */ error = dve_rom_load(user_filename, 0);
        else
        if (file_format == FMGR_FORMAT_KBD) /* Kbd */  error = psp_kbd_load_mapping(user_filename);
        else
        if (file_format == FMGR_FORMAT_SET) /* Settings */  error = dve_load_file_settings(user_filename);
        else
        if (file_format == FMGR_FORMAT_CHT) /* Cheat */  error = dve_load_file_cheat(user_filename);
      }
    }
