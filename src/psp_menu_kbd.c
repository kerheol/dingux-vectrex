...
psp_display_screen_kbd_menu(void)
{
  ...
  for (menu_id = 0; menu_id < MAX_MENU_KBD_ITEM; menu_id++)
  {
    ...
    if (menu_id == MENU_KBD_SKIN) {
      ...
      scan = strchr(buffer, '/');
      if (scan) *scan = 0;
      psp_sdl_back2_print(75, y, buffer, color);
    } else
        ...
        (menu_id <= MENU_KBD_JOY_RIGHT))
    {
      ...
      if (menu_kbd_selected == KBD_NORMAL_MAPPING  ) dve_key = loc_kbd_mapping[kbd_id];
      else
      if (menu_kbd_selected == KBD_LTRIGGER_MAPPING) dve_key = loc_kbd_mapping_L[kbd_id];
      else
      if (menu_kbd_selected == KBD_RTRIGGER_MAPPING) dve_key = loc_kbd_mapping_R[kbd_id];

      if ((dve_key >= 0) && (dve_key < DVE_MAX_KEY)) {
        strcpy(buffer, psp_dve_key_info[dve_key].name);
      } else
      if (dve_key == KBD_UNASSIGNED) {
        sprintf(buffer, "UNASSIGNED");
      } else
      if (dve_key == KBD_LTRIGGER_MAPPING) {
        sprintf(buffer, "L MAPPING");
      } else
      if (dve_key == KBD_RTRIGGER_MAPPING) {
        sprintf(buffer, "R MAPPING");
      } else {
        sprintf(buffer, "KEY %d", dve_key);
      }
...
psp_keyboard_menu_reset_kbd(void)
{
  ...
  psp_sdl_back2_print(175, 80, "Reset Keyboard !",
...
{
  psp_display_screen_kbd_menu();
  psp_sdl_back2_print(175, 80, "Set Hotkeys !",
                     PSP_MENU_WARNING_COLOR);
  psp_sdl_flip();
...
psp_keyboard_menu_load()
{
  ...
  {
    psp_display_screen_kbd_menu();
    psp_sdl_back2_print(175, 80, "File loaded !",
                       PSP_MENU_NOTE_COLOR);
    psp_sdl_flip();
    sleep(1);
  }
  ...
  {
    psp_display_screen_kbd_menu();
    psp_sdl_back2_print(175, 80, "Can't load file !",
                       PSP_MENU_WARNING_COLOR);
    psp_sdl_flip();
    sleep(1);
  }
...
psp_keyboard_menu_mapping(int kbd_id, int step)
{
  ...
  if (ploc_kbd_mapping[kbd_id] <  -3) ploc_kbd_mapping[kbd_id] = DVE_MAX_KEY-1;
  else
  if (ploc_kbd_mapping[kbd_id] >= DVE_MAX_KEY) ploc_kbd_mapping[kbd_id] = -3;
}
...
psp_keyboard_menu_save()
{
  ...
  {
    psp_display_screen_kbd_menu();
    psp_sdl_back2_print(175, 80, "File saved !",
                       PSP_MENU_NOTE_COLOR);
    psp_sdl_flip();
    sleep(1);
  }
  ...
  {
    psp_display_screen_kbd_menu();
    psp_sdl_back2_print(175, 80, "Can't save file !",
                       PSP_MENU_WARNING_COLOR);
    psp_sdl_flip();
    sleep(1);
  }
      ...
      if (danzeff_key > DANZEFF_START) {
        ...
        if (dve_key != -1) {
              ...
              (cur_menu_id <= MENU_KBD_JOY_RIGHT))
          {
            ...
            if (menu_kbd_selected == -1) loc_kbd_mapping[kbd_id] = dve_key;
            else
            if (menu_kbd_selected == KBD_LTRIGGER_MAPPING) loc_kbd_mapping_L[kbd_id] = dve_key;
            else
            if (menu_kbd_selected == KBD_RTRIGGER_MAPPING) loc_kbd_mapping_R[kbd_id] = dve_key;
          }
        }
