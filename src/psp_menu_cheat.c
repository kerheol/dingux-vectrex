...
psp_display_screen_cheat_menu(void)
{
  ...
  for (cheat_id = 0; cheat_id < DVE_MAX_CHEAT; cheat_id++) {
    ...
    if (a_cheat->type != DVE_CHEAT_NONE) {
      char enable = (a_cheat->type == DVE_CHEAT_ENABLE) ? 'X' : ' ';
...
{
  DVE_cheat_t* a_cheat = &DVE.dve_cheat[cur_cheat];
...
psp_cheat_menu_add_cheat()
{
  ...
  for (cheat_id = 0; cheat_id < DVE_MAX_CHEAT; cheat_id++) {
    DVE_cheat_t* a_cheat = &DVE.dve_cheat[cheat_id];
    if (a_cheat->type == DVE_CHEAT_NONE) num_free++;
  }
  if (! num_free) num_free = 1;

  int num_add = 0;
  for (addr = 0; addr < DVE_CHEAT_RAM_SIZE; addr++) {
...
psp_cheat_menu_enable_cheat()
{
  ...
  if (a_cheat->type == DVE_CHEAT_ENABLE) a_cheat->type = DVE_CHEAT_DISABLE;
  else                                    a_cheat->type = DVE_CHEAT_ENABLE;
}
...
{
  int cheat_id;
  int cur_type = DVE_CHEAT_NONE;

  for (cheat_id = 0; cheat_id < DVE_MAX_CHEAT; cheat_id++) {
    DVE_cheat_t* a_cheat = &DVE.dve_cheat[cheat_id];
    if (a_cheat->type != DVE_CHEAT_NONE) {
      if (cur_type == DVE_CHEAT_NONE) {
        if (a_cheat->type == DVE_CHEAT_ENABLE) cur_type = DVE_CHEAT_DISABLE;
        else                                   cur_type = DVE_CHEAT_ENABLE;
      }
...
{
  if (step == 1) {
    cur_cheat++; if (cur_cheat >= DVE_MAX_CHEAT) cur_cheat = 0;
  } else if (step == -1) {
    cur_cheat--; if (cur_cheat < 0) cur_cheat = DVE_MAX_CHEAT - 1;
  }
}
...
psp_cheat_menu_edit_list()
{
  ...
  sprintf(TmpFileName, "%s/cheat.txt", DVE.dve_home_dir);
...
{
  if (scan_prev_ram == 0) {
    scan_prev_ram = (uchar *)malloc( DVE_CHEAT_RAM_SIZE );
